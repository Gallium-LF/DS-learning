...
class AMGraph{ //we need the number of vertices fixed if we are using this approach
    ...
    public:
        ...
        AMGraph<T>* subgraph(T* vList, int n){ //ques 2
            // TODO
            AMGraph<T> *r=new AMGraph<T>(vList,n);
            for(int i=0;i<this->numVer;i++){
            	for(int j=0;j<this->numVer;j++){
            		if(this->adjMatrix[i][j]==1&&r->verList.locateEle(i+1)!=-1&&r->verList.locateEle(j+1)!=-1){
            			r->addEdge(i+1,j+1);
					}
				}
			}
        ...
        bool isBipartite(){ //ques 3
            // TODO
            if(this->numVer<2)return false;
            int judge[this->numVer];
            int i,j,tmp,k;
            for(i=0;i<this->numVer;i++){
            	judge[i]=0;
			}
			for(i=0;i<this->numVer;i++){
				if(judge[i]==0){
					...
					while(!q.isEmpty()){
						tmp=q.deQueue();
						if(judge[tmp]==1)k=2;
						else if(judge[tmp]==2)k=1;
						for(j=0;j<this->numVer;j++){
							if(this->adjMatrix[tmp][j]==1){
								if(judge[j]==0){
									judge[j]=k;
									q.enQueue(j);
								}
								else{
									if(judge[j]==judge[tmp])return false;
								}
							}
						}
					}
