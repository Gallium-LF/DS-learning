...
class BinarySearchTree{
    ...
    public:
        ...
        int numWaysToGetBST(int mod_num){
            // TO DO
            BinTree<T> *tmp=this->root->getRChild();
            int r=gettimes(tmp)%mod_num;
            return r;
        }
        long long int gettimes(BinTree<T> *r00t){
        	if(r00t==NULL)return 1;
        	long long int times=gettimes(r00t->getLChild())*gettimes(r00t->getRChild());
			long long int l=getnum(r00t->getLChild());
			long long int r=getnum(r00t->getRChild());
			long long int sum=jc(l+r)/(jc(l)*jc(r));
			return times*sum;
		}
        long long int getnum(BinTree<T> *r00t){
        	if(r00t==NULL) return 0;
        	return 1+getnum(r00t->getLChild())+getnum(r00t->getRChild());
		}
        long long int jc(int num){
        	long long int sum=1;
        	for(int i=2;i<=num;i++){
        		sum*=i;
			}
...
void test1(){
    ...
    bst.insertData(15);
     bst.insertData(2);
      bst.insertData(7);
     bst.insertData(12); bst.insertData(17);
      bst.insertData(1); bst.insertData(3); bst.insertData(6); bst.insertData(8);
       bst.insertData(11); bst.insertData(13); bst.insertData(16); bst.insertData(18);
